...
from app.tables import (
    ...
    coproductionprocess_tags_association_table,
)
...
class CoproductionProcess(BaseModel):
    ...
    status = Column(
        Enum(Status, create_constraint=False, native_enum=False),
    ...
    creator = relationship(
        ...
        post_update=True,
        backref="created_coproductionprocesses",
    )
    ...
    organization_id = Column(
        UUID(as_uuid=True),
        ForeignKey("organization.id", use_alter=True, ondelete="SET NULL"),
    )
    ...
    cloned_from_id = Column(
        UUID(as_uuid=True),
        ForeignKey(
            "coproductionprocess.id", ondelete="CASCADE", name="fk_copro_id_cloned_from"
        ),
        nullable=True,
    )
    cloned_to = relationship(
        "CoproductionProcess", backref=backref("cloned_from", remote_side=[id])
    )
    ...
    def enabled_teams(self):
        return [
            ...
            if not perm.treeitem or (perm.treeitem and not perm.treeitem.disabled_on)
        ]
